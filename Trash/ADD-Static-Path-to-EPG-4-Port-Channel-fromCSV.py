...
# This script can only create static paths for port-channel interface (regular/standalone ports not supported
# ####################################################################################################
...
ls = cobra.mit.session.LoginSession('https://10.0.100.10/', 'admin', 'Nisnis01')
md = cobra.mit.access.MoDirectory(ls)
md.login()

# parse the CSV file and loop over each line
with open('ADD-Static-Path-to-EPG-4-Port-Channel-fromCSV.csv') as csvfile:
    reader = csv.DictReader(csvfile, delimiter=';')
    for row in reader:
        print 'Adding the following Static Path to APIC'
        print 'TENANT: ' + str(row['TENANT'])
        print 'POD: ' + str(row['POD'])
        print 'LEAVES: ' + str(row['NODES'])
        print 'PG_NAME: ' + str(row['PG_NAME'])
        ...
        path2 = 'topology/pod-' + str(csv_pod) + '/protpaths-' + str(csv_node) + '/pathep-[' + str(csv_port) + ']'
